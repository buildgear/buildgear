...
int main (int argc, char *argv[])
{
   ...
   /* Generate manifest in xml format*/
   if (Config.show && Config.manifest_xml) {
	   Manifest.setFormat(FORMAT_XML);
	   Manifest.setHeader("Autogenerated by Build Gear v" VERSION " - Do not edit");
	   Manifest.setFooter("Autogenerated by Build Gear v" VERSION " - Do not edit");
	   if(Config.name_stripped == "") {
		   Manifest.generateManifest(NULL);
	   } else {
		   Manifest.generateManifest(BuildFiles.BuildFile(Config.name));
	   }

	   exit(EXIT_SUCCESS);
   }

   /* Generate manifest in plain text format*/
   if (Config.show && Config.manifest_plain_text) {
